...
# -------------------------
# Environment factory (NO ActionMasker - RecurrentPPO doesn't support it)
# -------------------------
def make_env():
...
class AoIEvalCallback(BaseCallback):
    ...
    def _run_evaluation(self):
        ...
        for _ in range(self.n_eval_episodes):
            ...
            while not done[0]:
                # RecurrentPPO predict with LSTM states
...
if __name__ == "__main__":
    ...
    # RecurrentPPO benefits from parallel envs
    n_envs = 4
    train_env = DummyVecEnv([make_env for _ in range(n_envs)])
    eval_env = DummyVecEnv([make_env])

    # Folders
    tb_dir = f"./tensorboard_logs/recurrent_ppo_{timestamp}/"
    best_dir = f"./models/recurrent_ppo_{timestamp}/"
    ckpt_dir = f"./checkpoints/recurrent_ppo_{timestamp}/"
    ...
    model.save(os.path.join(best_dir, "recurrent_ppo_drone_final"))
    print(f"Final model saved to {os.path.join(best_dir, 'recurrent_ppo_drone_final.zip')}")